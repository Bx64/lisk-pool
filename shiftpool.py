...
parser = argparse.ArgumentParser(description='SHIFT delegate pool script')
...
def estimatePayouts (log):
	uri = 'http://' + conf['node'] + '/api/delegates/forging/getForgedByAccount?generatorPublicKey=' + conf['pubkey'] + '&start=' + str (log['lastpayout']) + '&end=' + str (int (time.time ()))
	d = requests.get (uri)
	rew = d.json ()['rewards']
